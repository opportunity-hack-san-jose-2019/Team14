...
class Person:
    def __init__(self, full_name, first_name, last_name, cohort, evening, location, phone, email, interests, attendence, module_score, project_score, bonus, total_score):
        ...
        self.password = base64.b64encode(b'password')
...
with open('students.csv') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            email = row[7]
            interests = row[8]
            interests_list = [json.dumps(Skill(i.strip()).__dict__) for i in interests.split(';')]
            interests_list.pop()
            ...
            r = requests.post('https://obscure-badlands-88487.herokuapp.com/student/register', json=answers)
            print(r.status_code)
            line_count += 1